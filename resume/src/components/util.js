...
const BaseInfo = (props) => {
        ...
        switch (info.type) {
            case 'phone':
                return <li className="primary-info-list-item primary-info-list-item_phone" key={index}>
                    <i className="fa fa-phone-square" aria-hidden="true"></i>&nbsp;{info.content}</li>;
            // break;
            case 'blog':
                return <li className="primary-info-list-item primary-info-list-item_blog" key={index}>
                    <i className="fa fa-rss-square" ></i><a href={info.content} target="_blank" />&nbsp;{info.content}</li>;
            // break;
            case 'email':
                return <li className="primary-info-list-item primary-info-list-item_email" key={index}>
                    <i className="fa fa-envelope" aria-hidden="true"></i>&nbsp;{info.content}</li>;
            // break;
            default:
                return '';
            // break;
    ...
    return <section className="primary-info">
        <ul className="primary-info-list">
            {_.map(props.baseinfo.maininfo, function (info, index) {
                return <li key={index} className='primary-info-list-item'>{info}</li>
            })}
        </ul>
...
function Experience(props) {
    function parse(experience, index) {
        return <li className={'experience-list-item experience-list-item_' + experience.type} key={index}>
            {((url) => {
                if (url) return <img className='experience-list-item_img' src={url} alt='项目介绍' />
            })(experience.imgUrl)}
            <div>
                <h3 className="experience-list-item_title">{experience.title}</h3>
                <div className="experience-list-item_detail">
                    <p className="content">{experience.description}</p>
                    { ((link)=>{
                     if (link) return <footer className="ref"><a href={link} target='_blank'>Demo</a></footer>
                    })(experience.refLink)}
                    </div>
            </div>
        </li>
...
const Skill = (props) => {
    function parse(skill, index) {
        return <li className={'skill-list-item skill-list-item_' + skill.type} key={index}>
            <h3 className="skill-list-item_name" style={{ width: skill.level }}>{skill.title}</h3>
            <div className="skill-list-item_detail">
                <ul className="list content">
                    {_.map(skill.description, function (desc, index) {
                        return <li key={index}><i className="fa fa-check-circle-o" aria-hidden="true"></i>{desc}</li>
                    })}
                </ul>
            </div>
        </li>
    ...
    return (
        <section id='skill'>
            <div className='section-header'><h2 className="section-header_title">技能</h2></div>
            <div className="section-body">
                <ul className='skill-list--master'>
                    {_.map(props.skills.master, parse)}
                </ul>
                <ul className='skill-list--practised'>
...
const Appraisal = (props) => {
    return (
        <section id="appraisal">
            ...
            <div className="section-body">
                <p>{props.appraisal.content}</p>
            </div>
        </section>
    )
}
export { Experience, Skill, BaseInfo, Appraisal }
